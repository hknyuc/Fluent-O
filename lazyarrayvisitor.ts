...
export class LazyArrayVisitor extends ExpressionVisitor {
    ...
    private _getSource() {
        if (Utility.instanceof(this.source,Promise)) return this.source;
    ...
    */
    ...
    select(select: Select) {
        return this.getSource().then((source) => {
            let fn = (source, result, property) => {
                ...
                if (Utility.instanceof(baseValue,Promise)) {
    ...
    find(find: Find) {
        let getValueOf = function (value) {
            return value != null && Utility.instanceof(value.valueOf,Function) ? value.valueOf() : value;;
        }
        return this.getSource().then((source) => {
            let value = find.value;
            let result;
            if (typeof value !== "object" || (Utility.instanceof(value,Guid))) {
    ...
    modelMethod(modelMethod: ModelMethod) {
        return this.getSource().then((source) => {
            return this.createMemVisitor(source).visit(modelMethod.property).then((value) => {
                ...
                if (Utility.instanceof(value,Array)) {
    ...
    value(value: Value) {
        ...
        if (Utility.instanceof(value.value,Promise)) return fn(value.value);
    ...
    private static rangeExpressions(expressions: Array<any>): { expandAndSelects: Array<any>, others: Array<any> } {
        ...
        filters = filters.length > 1 ? [new Filter(filters.reduce((accumulator, current) => {
            if (Utility.instanceof(accumulator,Filter)) {
    ...
    static getOnlyStucts(element) {
        ...
        for (let i in element) {
            ...
            let isObject = validsObject.some(v => Utility.instanceof(element[i],v));
    ...
    private static filterExpressions(expressions: Array<any>, type: any) {
        return expressions.filter(a => Utility.instanceof(a,type));
    }
