...
class LazyArrayVisitor extends expressions_1.ExpressionVisitor {
    ...
    _getSource() {
        if (core_1.Utility.instanceof(this.source, Promise))
            return this.source;
    ...
    */
    select(select) {
        return this.getSource().then((source) => {
            let fn = (source, result, property) => {
                ...
                if (core_1.Utility.instanceof(baseValue, Promise)) {
    ...
    find(find) {
        let getValueOf = function (value) {
            return value != null && core_1.Utility.instanceof(value.valueOf, Function) ? value.valueOf() : value;
            ;
        };
        return this.getSource().then((source) => {
            let value = find.value;
            let result;
            if (typeof value !== "object" || (core_1.Utility.instanceof(value, schema_1.Guid))) {
    ...
    modelMethod(modelMethod) {
        return this.getSource().then((source) => {
            return this.createMemVisitor(source).visit(modelMethod.property).then((value) => {
                ...
                if (core_1.Utility.instanceof(value, Array)) {
    ...
    value(value) {
        ...
        if (core_1.Utility.instanceof(value.value, Promise))
   ...
   */
    static rangeExpressions(expressions) {
        ...
        filters = filters.length > 1 ? [new expressions_1.Filter(filters.reduce((accumulator, current) => {
                if (core_1.Utility.instanceof(accumulator, expressions_1.Filter)) {
    ...
    static getOnlyStucts(element) {
        ...
        for (let i in element) {
            ...
            let isObject = validsObject.some(v => core_1.Utility.instanceof(element[i], v));
    ...
    static filterExpressions(expressions, type) {
        return expressions.filter(a => core_1.Utility.instanceof(a, type));
    }
    isDataSet(dataSetable) {
        let result = dataset_1.DataSet.is(dataSetable);
        return result;
    }
